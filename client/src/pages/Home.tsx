...
function ProductVisual({ product, detail = false }: { product: Product; detail?: boolean }) {
  ...
  return <div className={`${detail ? "detail-visual" : "product-image-wrap"} product-${product.slug}`}>
    <div className="visual-fallback" role="img" aria-label={visualLabel} data-visual-source="Eryndex product signal diagram"><span className="fallback-status">EDX / {product.number} · SIGNAL READY</span><svg className="product-motion-overlay" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
      {product.slug === "qadryn" && <g className="product-symbol-visual visual-qadryn"><path id={`${motionId}-route`} className="process-route" d="M 14 71 C 23 54 28 31 44 35 S 66 59 84 29" /><path className="process-branch" d="M 28 43 C 40 49 50 64 66 75" /><path className="process-branch" d="M 44 35 C 54 27 66 28 78 42" /><g className="process-nodes"><circle cx="14" cy="71" r="2.2" /><circle cx="28" cy="43" r="2.2" /><circle cx="44" cy="35" r="2.8" /><circle cx="66" cy="75" r="2.2" /><circle cx="78" cy="42" r="2.2" /><circle cx="84" cy="29" r="2.8" /></g><circle className="process-dot" r="1.65"><animateMotion dur="5.6s" repeatCount="indefinite" rotate="auto"><mpath href={`#${motionId}-route`} /></animateMotion></circle></g>}
      {product.slug === "vessyra" && <g className="product-symbol-visual visual-vessyra"><path className="shield-layer shield-outer" d="M 50 13 L 82 30 L 75 67 Q 50 88 25 67 L 18 30 Z" /><path className="shield-layer shield-middle" d="M 50 23 L 73 35 L 68 62 Q 50 77 32 62 L 27 35 Z" /><path className="shield-layer shield-inner" d="M 50 33 L 64 41 L 61 57 Q 50 66 39 57 L 36 41 Z" /><path className="shield-scan" d="M 22 52 H 78" /><path className="shield-scan scan-two" d="M 31 69 H 69" /></g>}
