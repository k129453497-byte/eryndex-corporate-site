// Eryndex Signal Atelier｜頁面內容與互動。採資料流長頁、偏移分欄與系統標籤，所有主要文案均以三語資料結構維護。
...
const faqs = [
  { q: { "zh-TW": "Eryndex 適合什麼規模的企業？", "zh-CN": "Eryndex 适合什么规模的企业？", en: "What size of business is Eryndex for?" }, a: { "zh-TW": "Eryndex 聚焦中小型企業與成長型團隊，提供可從單一流程開始、再逐步擴展的模組化工具。實際適配度仍會依資料環境、流程複雜度與治理需求評估。", "zh-CN": "Eryndex 聚焦中小型企业与成长型团队，提供可从单一流程开始、再逐步扩展的模块化工具。实际适配度仍会依据数据环境、流程复杂度与治理需求评估。", en: "Eryndex is designed for small and medium businesses and growing teams. Each module can start with one workflow and expand over time; fit depends on your data environment, process complexity, and governance needs." } },
  ...
  { q: { "zh-TW": "產品是否支援企業內部部署？", "zh-CN": "产品是否支持企业内部部署？", en: "Do you support on-site deployment?" }, a: { "zh-TW": "Terviq 是為企業內部 AI 處理需求設計的產品方向。部署方式、硬體規格與資料邊界會依企業環境進行技術評估，不以單一方案套用。", "zh-CN": "Terviq 是为企业内部 AI 处理需求设计的产品方向。部署方式、硬件规格与数据边界会依据企业环境进行技术评估，不以单一方案套用。", en: "Terviq is designed for on-site AI processing. Deployment, hardware sizing, and data boundaries are assessed against each environment rather than prescribed as one fixed setup." } },
...
function HeroConsole() {
  ...
  return <div className="hero-console" aria-label="Decorative operational intelligence console">
    <div className="console-top"><span className="console-kicker">EDX / SIGNAL CORE</span><span className="console-live"><i /> {small}</span></div>
...
function ProductVisual({ product, detail = false }: { product: Product; detail?: boolean }) {
  ...
  return <div className={`${detail ? "detail-visual" : "product-image-wrap"} product-${product.slug}`}>
    <div className="visual-fallback" role="img" aria-label={visualLabel} data-visual-source="Eryndex product signal diagram"><span className="fallback-status">EDX / {product.number} · SIGNAL READY</span><svg className="product-motion-overlay" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
      {product.slug === "qadryn" && <g className="product-symbol-visual visual-qadryn"><path id={`${motionId}-route`} className="process-route" d="M 14 71 C 18 60 23 48 28 43 C 34 38 38 36 44 35 C 54 32 65 38 78 42 C 81 39 82 33 84 29" /><path className="process-branch" d="M 28 43 C 40 49 50 64 66 75" /><path className="process-branch" d="M 44 35 C 54 27 66 28 78 42" /><g className="process-nodes"><rect x="11.8" y="68.8" width="4.4" height="4.4" rx=".7" /><rect x="25.8" y="40.8" width="4.4" height="4.4" rx=".7" /><rect x="41.2" y="32.2" width="5.6" height="5.6" rx=".8" /><rect x="63.8" y="72.8" width="4.4" height="4.4" rx=".7" /><rect x="75.8" y="39.8" width="4.4" height="4.4" rx=".7" /><rect x="81.2" y="26.2" width="5.6" height="5.6" rx=".8" /></g><circle className="process-dot" r="1.65"><animateMotion dur="5.6s" repeatCount="indefinite" calcMode="linear" keyPoints="0;.18;.18;.43;.43;.82;.82;1" keyTimes="0;.16;.27;.43;.54;.82;.91;1" rotate="auto"><mpath href={`#${motionId}-route`} /></animateMotion></circle></g>}
      {product.slug === "vessyra" && <g className="product-symbol-visual visual-vessyra"><path className="shield-layer shield-outer" d="M 50 13 L 82 30 L 75 67 Q 50 88 25 67 L 18 30 Z" /><path className="shield-layer shield-middle" d="M 50 13 L 80 39 L 70 71 L 50 87 L 30 71 L 20 39 Z" /><path className="shield-layer shield-inner" d="M 50 13 C 69 18 80 31 80 46 C 80 66 67 78 50 87 C 33 78 20 66 20 46 C 20 31 31 18 50 13 Z" /><path className="shield-scan" d="M 22 52 H 78" /><path className="shield-scan scan-two" d="M 31 69 H 69" /></g>}
  ...
  return <>
    ...
    <section className="hero-section">
      ...
      <div className="container hero-layout">
        <div className="hero-copy"><Eyebrow index="EDX / 00">{lang === "en" ? "Intelligence for everyday operations" : lang === "zh-CN" ? "为日常运营而生的智能" : "為日常營運而生的智慧"}</Eyebrow><h1>{lang === "en" ? <>Let technology<br /><em>understand work.</em></> : lang === "zh-CN" ? <>让科技<br /><em>理解工作。</em></> : <>讓科技<br /><em>理解工作。</em></>}</h1><p className="hero-lede">{lang === "en" ? "Eryndex helps small and medium businesses automate the repeatable, protect the important, and see what comes next." : lang === "zh-CN" ? "Eryndex 协助中小企业自动化重复工作、保护重要资料，并看见下一步。" : "Eryndex 協助中小企業自動化重複工作、保護重要資料，並看見下一步。"}</p><div className="hero-actions"><PrimaryButton href="/products">{lang === "en" ? "Explore products" : lang === "zh-CN" ? "探索产品" : "探索產品"}</PrimaryButton><SecondaryButton href="/contact">{lang === "en" ? "Book a demo" : lang === "zh-CN" ? "预约演示" : "預約展示"}</SecondaryButton></div><div className="hero-note"><span /> {lang === "en" ? "Built for the work behind the work." : lang === "zh-CN" ? "为工作背后的工作而设计。" : "為工作背後的工作而設計。"}</div></div>
        <div className="hero-art"><div className="hero-orbit orbit-a"><span className="hero-signal-dot dot-green" /></div><div className="hero-orbit orbit-b"><span className="hero-signal-dot dot-violet" /></div><img src="/manus-storage/eryndex-ai-signal_4f3d1669.png" alt={lang === "en" ? "Abstract enterprise intelligence network with connected data signals" : lang === "zh-CN" ? "连接数据讯号的抽象企业智能网络" : "連結資料訊號的抽象企業智慧網路"} /><div className="hero-art-overlay" /><HeroConsole /><div className="hero-coordinate">25°02' N<br />121°33' E</div></div>
      </div>
      <div className="container hero-foot"><span>Eryndex / INTELLIGENCE SYSTEMS</span><span className="scroll-cue"><i /> {lang === "en" ? "Scroll to map the system" : lang === "zh-CN" ? "向下查看系统" : "向下查看系統"}</span></div>
    </section>
    ...
    <section className="principle-section"><div className="container principle-grid"><SignalRail label="EDX / PRINCIPLE" /><div className="principle-copy"><Eyebrow index="02">{lang === "en" ? "Our point of view" : lang === "zh-CN" ? "我们的观点" : "我們的觀點"}</Eyebrow><h2>{lang === "en" ? "Technology should return attention to people." : lang === "zh-CN" ? "科技應該把注意力還給人。" : "科技應該把注意力還給人。"}</h2><p>{lang === "en" ? "We design systems that reduce noise without hiding the reasoning. The result is not more dashboards to maintain, but more room for teams to decide, create, and grow." : lang === "zh-CN" ? "我们设计能减少噪音、却不隐藏判断过程的系统。结果不是更多需要维护的仪表板，而是让团队拥有更多时间去决策、创造与成长。" : "我們設計能減少噪音、卻不隱藏判斷過程的系統。結果不是更多需要維護的儀表板，而是讓團隊擁有更多時間去決策、創造與成長。"}</p><Link href="/about" className="arrow-link">{lang === "en" ? "See how we think" : lang === "zh-CN" ? "了解我们的想法" : "了解我們的想法"}<ArrowRight size={16} /></Link></div><div className="principle-mark" aria-hidden="true"><div className="mark-core"><Sparkles size={28} /></div><span className="principle-signal-dot dot-green" /><span className="principle-signal-dot dot-violet" /><span className="mark-label">SIGNAL<br />BECOMES<br />CONTEXT</span></div></div></section>
    <section className="solutions-preview section-dark"><div className="container"><div className="section-heading split-heading"><div><Eyebrow index="03">{lang === "en" ? "Designed around needs" : lang === "zh-CN" ? "围绕需求设计" : "圍繞需求設計"}</Eyebrow><h2>{lang === "en" ? <>A practical path<br /><em>into AI.</em></> : lang === "zh-CN" ? <>一条务实的<br /><em>AI 之路。</em></> : <>一條務實的<br /><em>AI 之路。</em></>}</h2></div><Link href="/solutions" className="arrow-link">{lang === "en" ? "View solutions" : lang === "zh-CN" ? "查看解决方案" : "查看解決方案"}<ArrowUpRight size={16} /></Link></div><div className="solution-list">{solutions.map((solution) => { const Icon = solution.icon; return <Link href={`/products/${solution.product}`} className="solution-row" key={solution.id}><span className="solution-number">{solution.number}</span><span className="solution-icon"><Icon size={20} strokeWidth={1.5} /></span><span className="solution-title">{tx(solution.title, lang)}</span><span className="solution-text">{tx(solution.text, lang)}</span><ArrowUpRight className="solution-arrow" size={18} /></Link>; })}</div></div></section>
    <HomeCTA />
  </>;
}
...
function PageHeader({ eyebrow, title, intro, index = "00" }: { eyebrow: Copy; title: ReactNode; intro: Copy; index?: string }) {
  const { lang } = useSite();
  return <section className="page-header"><div className="container page-header-grid"><SignalRail label={`EDX / ${index}`} /><div><Eyebrow index={index}>{tx(eyebrow, lang)}</Eyebrow><h1>{title}</h1><p className="page-intro">{tx(intro, lang)}</p></div></div></section>;
}
...
export function ProductDetailPage() {
  ...
  return <><Meta title={`${tx(product.name, lang)} | Eryndex`} description={tx(product.summary, lang)} /><section className={`product-detail-hero tone-${product.tone}`}><div className="container detail-hero-grid"><div className="detail-copy"><Link href="/products" className="back-link"><ArrowRight size={15} className="back-arrow" />{lang === "en" ? "All products" : lang === "zh-CN" ? "全部产品" : "全部產品"}</Link><Eyebrow index={`${product.number} / 04`}>{tx(product.category, lang)}</Eyebrow><div className="detail-icon"><Icon size={28} strokeWidth={1.4} /></div><h1>{tx(product.name, lang)}</h1><h2>{tx(product.strap, lang)}</h2><p>{tx(product.summary, lang)}</p><PrimaryButton href="/contact">{lang === "en" ? "Book a product demo" : lang === "zh-CN" ? "预约产品演示" : "預約產品展示"}</PrimaryButton></div><ProductVisual product={product} detail /></div></section><section className="detail-body"><div className="container detail-body-grid"><div className="detail-sticky"><Eyebrow index="01">{lang === "en" ? "Why it exists" : lang === "zh-CN" ? "为什么存在" : "為什麼存在"}</Eyebrow><h2>{lang === "en" ? "The problem is not a lack of tools. It is a lack of context." : lang === "zh-CN" ? "问题不是缺少工具，而是缺少脉络。" : "問題不是缺少工具，而是缺少脈絡。"}</h2></div><div className="detail-content"><p className="detail-problem">{tx(product.problem, lang)}</p><div className="detail-section"><p className="mini-label">EDX / CAPABILITIES</p><h3>{lang === "en" ? "A system that stays legible as work moves." : lang === "zh-CN" ? "在工作流动时，系统依然清晰。" : "在工作流動時，系統依然清晰。"}</h3><ul className="feature-list">{list(product.features, lang).map((feature) => <li key={feature}><Check size={17} />{feature}</li>)}</ul></div><div className="detail-section usecase-section"><p className="mini-label">EDX / IN PRACTICE</p><h3>{lang === "en" ? "Where teams begin" : lang === "zh-CN" ? "团队从这里开始" : "團隊從這裡開始"}</h3><div className="usecase-grid">{list(product.useCases, lang).map((useCase, index) => <div className="usecase-item" key={useCase}><span>0{index + 1}</span><strong>{useCase}</strong></div>)}</div></div><div className="detail-section"><p className="mini-label">EDX / FOR WHOM</p><p className="audience-note">{tx(product.audience, lang)}</p></div></div></div></section><section className="other-products section-dark"><div className="container"><div className="section-heading"><Eyebrow index="02">{lang === "en" ? "Continue exploring" : lang === "zh-CN" ? "继续探索" : "繼續探索"}</Eyebrow><h2>{lang === "en" ? <>One signal rarely<br /><em>stands alone.</em></> : lang === "zh-CN" ? <>一个讯号很少<br /><em>独自发生。</em></> : <>一個訊號很少<br /><em>獨自發生。</em></>}</h2></div><div className="related-grid">{otherProducts.slice(0, 2).map((item) => <ProductCard key={item.slug} product={item} />)}</div></div></section><HomeCTA /></>;
}
